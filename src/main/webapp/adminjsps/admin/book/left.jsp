 ...
 * 1. 对象名必须与第一个参数相同！
   2. 第二个参数是显示在菜单上的大标题
 */
var bar = new Q6MenuBar("bar", "網上書城");
$(function() {
	...
	*/
    ...
    <c:forEach items="${parents}" var="parent">
      <c:forEach items="${parent.children}" var="child">
        ...
        --%>
	bar.add("圖書管理", "查看全部圖書", "<c:url value='/admin/AdminBookServlet/findAll'/>", "body");
	<c:forEach items="${requestScope.categoryList}" var="category">
		bar.add("圖書管理", "${category.cname}", "${pageContext.request.contextPath}/admin/AdminBookServlet/findByCategory/${category.cid}", "body");
	</c:forEach>
